...
Seven core service singletons registered in `project.godot` (plus `SceneManager`, `NotepadService`, `FoundersMiningService`, `CasinoScBalanceService`, `CasinoClientLedgerService`, `PlayerBankAccountService`, `BtcMarketDataService`, and `CasinoCoinSwapService`, documented in their own sections — **fifteen autoloads total**). They persist across all scenes and are accessible globally by class name. (`BtcMarketDataService` — Step 13 MD.1, historical BTC/USD daily data + day-change events — belongs to `AIHelperFiles/step13-btc-market-data-and-dev-alt-timeline-plan.md`; its own dedicated subsection lands with that plan's TL.3 docs pass.)
...
│   ├── Services/               # Autoload singletons (15 services)
