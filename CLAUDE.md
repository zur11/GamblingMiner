...
Owns the **historical BTC network dataset** (Step 14 ND.1, autoload #17) — loader + O(1) lookup service for `Data/HistoricalNetwork/btc_network_daily_2009_2025.csv` (6,207 rows, one per UTC day, **2009-01-03 → 2025-12-31**; Coin Metrics community tier, cross-checked against blockchain.com; provenance/caveats in `AIHelperFiles/step14-historical-network-population-scheduler-plan.md` §2/ND.0). Mirrors `BtcMarketDataService` exactly: CSV loaded once (`EnsureLoaded()`, idempotent — also callable from a throwaway, never-scene-tree instance, the pattern EB.1's entry-year bootstrap uses), day-indexed array, O(1) `TryGetDay`, a `NetworkDayChanged` day-boundary event, read-only over a static asset (no persistence, no checkpoint coverage, no reset-list entry needed).
...
│   ├── RecruitableBiddingDetails/ # Per-non-miner tracked-donation pool + settlement summary (Step 14 ND.5)
...
- **Network fee market simulation — direction LOCKED (2026-07-07, Step 14 / D-14.6), implementation pending**: `NetworkFeePolicy.MinFee` is still a single hardcoded `0.1 BTC` constant, but the model decision is made — **Option A (historical fee replay, mirroring `BtcMarketDataService`'s price-history architecture) is the chosen mechanism**; the historical fee columns ship inside Step 14's network dataset (fees are fractal-exempt like `price_usd` — replayed at face value, never /100). **Option B** (a reactive fee market from our own simulated mempool congestion) is retained as a **future validation experiment**: if it later reproduces a curve similar to the replay, it confirms the Step-14 population/volume simulation was built right. Making `NetworkFeePolicy` actually consume the data is its own future step. See `AIHelperFiles/step14-historical-network-population-scheduler-plan.md` (D-14.6, §9.4) and `Documentation/PRIVATE_ROADMAP.md` "Network Fee Market Simulation" (decision recorded there).
...
│   │   └── RecruitableBiddingDetails (Step 14 ND.5, from Enroll Mode's "Details →") → BlockExplorer
