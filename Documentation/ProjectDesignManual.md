...
**The ladder (D-ND6.4).** Exact-tier re-bid probabilities, consecutive Fibonacci percentages, kept as a literal constant table (`ReBidProbabilityPercentByTier`) — never derived from a formula:

```
tier 4 → 5%    tier 5 → 8%    tier 6 → 13%    tier 7 → 21%    tier 8 → 34%    tier 9 → 55%
```

Tiers 1–3 have no entry — that is the **satisfied** state (a bot holding any top-3 slot in a pool never re-bids there). Tier 10 has no entry either: a bot whose best slot is tier 10 necessarily holds the smallest slot of a full pool, which the self-eviction guard (below) excludes before any roll — the sequence's natural next value (89) was removed as dead weight by developer decision at ND.6c; restore `[10] = 89` only if that guard is ever relaxed.
