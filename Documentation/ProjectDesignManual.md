...
- **UI**: `UI/DevTimeScaleSelector/DevTimeScaleSelector.cs` (programmatic, like `StatusBar`) — in DiceGame (next to the APS selector) and BlockExplorer (under the StatusBar). Live; **not persisted** (resets to the base scale on restart). The offered steps are `DevTimeScaleSelector.Multipliers` and are **not** enumerated here (Standing Convention 15); the ladder has two régimes — a fine range in 100X steps up to the coarse range's first step, then the coarse range in 1000X steps up to the clock's ceiling. The fine range was added **2026-08-30** for mini-plan 08: the throughput frontier is `credits × DevTimeScale`, so at high credit counts the highest sustainable scale lands inside it, and with only the coarse range on offer the frontier fell in a gap the selector could not express. It deliberately spans the **whole** first decade rather than stopping at the frontier today's `MaxBetsPerFrame` predicts — raising that constant (mini-plan 08 P1's purpose) moves the frontier, and a ladder cut to the current knee would reopen the same gap one measurement later.
- **Caveat**: `SimulationService.MaxBetsPerFrame` per node per frame caps throughput at `MaxBetsPerFrame × fps` bets/s/node; past that, acceleration stops being linear (the measured dynamics stay intact). The 10000X option was removed for hitting this ceiling; **9000X is the tested-smooth ceiling**. Irrelevant for the normal measurement regime (power split across low-rate nodes). **The constant was `10` from its introduction until 2026-08-30, when mini-plan 08 P1 priced a bet for the first time and raised it to `20`** — the value is cited by symbol here deliberately (Standing Convention 15), because it is now derived from a measurement that will be taken again.
