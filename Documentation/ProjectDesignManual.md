...
### 22.10 — The Saturation Ladder: casino-bot re-bidding refinement (Step 14 ND.6, 2026-07-12; ND.8d bid-count-aware refinement 2026-07-20)
...
**Future debt (documented, not built).** Move the whole bid system into internal casino wallets that then broadcast to the chain, so the contest is manageable without waiting for the next mined block to read the standings — the per-block-resolved warnings + this cleanup are the interim stopgap. Full diagnosis, matrix and decision log: `AIHelperFiles/step14-historical-network-population-scheduler-plan.md` §12.5.5.
