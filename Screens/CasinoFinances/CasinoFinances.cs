...
public partial class CasinoFinances : Control
{
	...
	private void RefreshTransactions()
	{
		...
		foreach ((long unixMs, string kind, decimal amount, string counterparty, int recipients, string memo) in history)
		{
			...
			// Multi-output sends (pool distributions) name the first payee + how many more; swap-desk txs
			// (memo "swap:…") get their own color so they never read as ordinary transfers (Step 13 / SW.3).
