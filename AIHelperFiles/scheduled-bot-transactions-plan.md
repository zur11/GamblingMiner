...
> **⚠️ Canonically amended (2026-07-09, Step 14 EB.2 — D-EB.4/5/6/7, retuned same day at round 3 — D-EB.8/9/10; see `step14-historical-network-population-scheduler-plan.md` §5.2–5.3, §6 / ProjectDesignManual §22.6):** introduction is now gated on **Market Birth (2010-07-18)** along the historical active-address curve (retiring `NonMinerIntroIntervalMs`), with the pool raised **10 → 40** (all 40 deployed by 2017-12-13); the window is **100 in-game days** (confirmed, superseding both the original 7-day value and an interim 6-month provisional one); the countdown starts at the bot's **FIRST QUALIFYING BID**, not its creation/introduction — never-bid-on bots stay recruitable indefinitely and every resolved auction has a winner; and **only casino players can bid (D-EB.7)** — turns out closer to this section's ORIGINAL OQ-4 than the assistant's first EB.2 draft assumed: eligibility = mining that requires betting at the casino, so **the player AND the classic casino-miner-bots `bot_1..4` both qualify** (they already run real hardware-credit-locked betting sessions); the much larger, historically-growing Step-14 cast of miners (up to 29 additional / 33 total by 2025 — mines via drained attempts, never bets) does not. OQ-1/OQ-4 below and the "Auction Window Rule" section remain the historical starter record (still `..non_miner_10` throughout — current pool size is 40), now closer to current canon on bidder eligibility than on window/countdown mechanics.
...
> **✅ Reconfirmed, not superseded, by D-EB.7 (2026-07-09)**: this OQ-4 resolution — `bot_1..4` compete for non-miner referrals just like the player — is exactly what the EB.2 rework restores after briefly narrowing bidding to the player alone. The distinguishing test D-EB.7 landed on (mining that requires betting at the casino) happens to draw the line right at `bot_1..4` — the same four bots this OQ already named. What's genuinely NEW past this OQ-4: the much larger Step-14 CAST of miners (ND.2, up to 33 by 2025) that did not exist when this was written — they mine via drained attempts, never bet, and so do NOT compete for referrals, unlike `bot_1..4`.
...
> **⚠️ Superseded (2026-07-09)** by the Step 14 EB.2 amendment (Market-Birth gate · 6-month window · first-donation countdown) — see the amendment box under OQ-1 above and ProjectDesignManual §22.6.
