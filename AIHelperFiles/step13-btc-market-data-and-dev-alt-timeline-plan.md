...
**Status**: v2 (2026-07-05) — **round-1 decisions LOCKED** (§0.5). Implementation not started; ready for the feature branch. The CSV asset already exists in the working tree (untracked).
...
- [ ] Extend the delete list to the full Step-11/12 state set + bet-history chunks for BOTH trigger paths (§3.4, D-13.7).
- [ ] Test: fake a stale tag ⇒ clean re-bootstrap; matching tag ⇒ untouched.
...
- [ ] Null-safe parsing (blank ≠ 0), InvariantCulture, halt-day carry-forward, pre-2010 nulls.
- [ ] /100 fractal accessors (D-13.10): `GetGameVolumeBtc` / `GetGameNumTrades` (÷100, floor-to-1); raw values reserved for DEV/provenance readouts.
- [ ] DEV smoke readout (e.g. temporary label or console print of current MarketDay on day change).

### Phase MD.2 — minimal player-visible surfacing
- [ ] BTCWallet valuation line + StatusBar ticker (D-13.3 a/b).
- [ ] ScFinances: dual-mode BTC/net-worth toggle label per §4.2-c + visual-weight upgrade of the SC-only game-over metric (D-13.3 c).
- [ ] Locked-state message before 2010-07-18 (canon worlds).

### Phase TL.2 — the simulacrum flip (BRANCH-ONLY — never merges to `main` as `true`)
- [ ] On the swaps branch: `DevAltTimeline = true`.
- [ ] Validate §3.5 acceptance criteria (bootstrap replay, founder arcs, fee-from-landing, market data live on landing day).
- [ ] Fees active from landing day (D-13.9): first player-era block after 2010-07-18 carries fees for all participants; bootstrap blocks fee-free.
- [ ] Hearn round-trip in the fee era: E6…E7b confirm with historical amounts intact (§3.6 check).
- [ ] Add a permanent visible DEV watermark while `DevAltTimeline` is true (e.g. StatusBar suffix `[ALT-TIMELINE DEV]`) so no screenshot/session can be mistaken for canon.

### Phase SW.\* — swap tooling (separate design round)
- [ ] Developed on the branch against the simulacrum; framing already locked by D-13.2/13.4/13.6 (§4.3); gets its own detailed design round (likely its own plan or a §4.3 upgrade) covering order form, spread knob, inventory seeding, ledger integration.

### Phase TL.3 — exit the simulacrum (merge-back gate)
- [ ] `DevAltTimeline` → `false`; confirm `Tag` reverts; launch ⇒ guard wipes the alt world ⇒ pristine canon bootstrap.
- [ ] Re-verify swaps in canon: desk locked at player start, unlocks when the clock crosses 2010-07-18 (fast-forward via DevTimeScale).
- [ ] Docs truth pass: CLAUDE.md (autoload #14, trading-unlock canonical decision, timeline guard), ProjectDesignManual, GLOSSARY (Swap/Market terms), this plan → COMPLETE.
