...
**Status**: ✅ **STEP 4 COMPLETE** (compiles clean; 4a + 4b.1 verified in-engine, 4b.2/4b.3/4c pending final in-engine pass). All 9 decisions resolved (§4). Slices: 4a ✅, 4b.1 ✅, 4b.2 ✅, 4b.3 ✅ (content-hash txid), 4c ✅ (BlockExplorer surfacing + player fee selector in BTCWallet). Next roadmap lead: **Step 6** (Step 5 was absorbed — see note).

> **Step 5 (refit bootstrap to the engine) absorbed into Step 4:** because 4b.1 replaced the mining core *in place* (`MinePendingTransactions`/`TryMineSingleNonceAttempt` now build via `BlockTemplateBuilder`), the historical bootstrap and weighted lottery already mine real candidate blocks through the new engine — no separate simplified path remained to refit. Step 5 is therefore verify-only.
...
6. **Fees end-to-end** ✅ DONE (4b.2) — `Transaction.Fee` is now part of the signed payload (`BuildTransactionPayload`); the sender pays `Amount + Fee` (enforced in `AddTransactionToPendingTransactions` and reflected in `GetAddressData`/`GetAddressSpendableBalance`); the miner collects `Σ Fee` via the coinbase (`BlockTemplateBuilder`, from 4b.1). `NodeAgent.CreateSignedTransaction` and the `NetworkRoot` broadcast methods take a `fee` (default 0). Miner bots attach a random `0.1–1.0 BTC` fee (`scheduled-bot-transactions` Phase 4 reactivated). **Money is conserved:** sender −(A+F), recipient +A, miner +F. **Player-facing fee selector in the send UIs is deferred to 4c** (engine supports it; UI sends currently pass fee 0).
7. **Dev/verify** ✅ DONE (4c) — BlockExplorer block lookup + latest-block view show the **Merkle root**, block **time**, **fees collected**, and per-tx **fee** with a **[COINBASE]** marker; tx lookup shows fee too. Confirmed in-engine that a slower-but-luckier player can beat a faster bot to a block → candidates are genuinely per-miner. A **player-facing fee selector** was added to **BTCWallet**'s send form (the dev wallets — Casino/Founders/Bots — send with fee 0, which is fine; they're not player-facing).

### Content-hash txid (4b.3) ✅ DONE

`BlockchainService.ComputeTransactionId(tx)` = double-SHA256 of canonical content (`Amount|Sender|Recipient|Fee|InputDataHex|IsSpendable|Salt`). `MerkleTree.LeafHash` now returns this (id and Merkle leaf are the same value). `Transaction.Salt` is a uniqueness nonce — random for normal txs (`CreateUnsignedTransaction`), `coinbase:{height}` for coinbases (BIP34-style) — so identical payments/equal coinbases get distinct ids. `NodeAgent.CreateSignedTransaction` sets the id after the fee is known; `ValidateTransactionSignature` rejects any non-coinbase tx whose id ≠ its content hash. Genesis (`genesis-coinbase`) and the block-2 bootstrap tx keep their semantic sentinel ids (unique, unsigned, used for dedup); their Merkle leaf is still the recomputed content hash.
