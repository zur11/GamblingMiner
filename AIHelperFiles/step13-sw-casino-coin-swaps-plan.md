...
- [ ] `OfferedBtc`/`OfferedSc`/`IsPanelAEnabled`/`IsPanelBEnabled` + `DisableReason`, recomputed on the §1.1 event set.
- [ ] **Verify the simulacrum funding timeline**: confirm the casino actually accrues its first BTC (pool mining) and first SC (player losses) fast enough for dev iteration; if BTC is too slow, add the DEV seed fallback (D-SW.7 — alternative, not a priority).

### SW.2 — the `CasinoCoinSwaps` scene (display-only) + DEV rows in the two casino scenes
- [ ] **Read ProjectDesignManual Ch. 29, then** build the §6 layout; SceneManager entry; MainMenu + ScFinances links; origin-aware Back.
- [ ] Both panels render availability, offered/reserve readouts, quote previews (pure `Quote*` calls), enable/disable states with reasons, halt-day and no-market states. Header shows the current fee read-only. No DEV controls in this scene (D-SW.9 as amended).
- [ ] `CasinoGamblingFinances` gains the swap-desk DEV row (D-SW.9): swap-fee SpinBox (default 10%, clamp 1–10%, both directions) + SC swap-reserve selector (%/amount toggle on casino Main Balance, default 0) — adapted from / replacing the old spread-knob idea, wired to the service setters.
- [ ] `CasinoFinances` (the casino's BTC wallet scene) gains the **BTC swap-reserve panel** (D-SW.9): %/amount toggle on the casino's whole BTC wallet total, default 0, tagged `[DEV]`, wired to the same service setters. (Ch. 29 applies here too if the scene's scroll layout is touched.)
- [ ] SWAP buttons present but disabled ("execution lands in SW.3/SW.4") — lets the first goal (tools enable/disable on funds) be tested end-to-end before any money moves.
