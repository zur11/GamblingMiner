...
- **ND.0 — dataset build + cross-check + commit**: run the two scripts, assert continuity + provider agreement, developer commits the CSV (git policy). Pin the `.csv.import` to `importer="keep"` **from day one** (the MD.1 lesson — Godot silently imports any CSV as a Translation resource).
