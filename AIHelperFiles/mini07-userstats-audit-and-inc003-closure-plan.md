...
| **G4 — D1 chooses "wipe now"** and the wipe is executed before Phase B | ✅ **FIRED, 2026-08-23 — at step 5 of this plan's own order of work** (`a0c27ea`, `WorldFormatVersion` 5 → 6). **Phase B proceeds unchanged** — it wants a virgin world anyway (mini-plan 06 §3.2). The wipe is a precondition it already asked for, and it is now met rather than pending. **Revised:** the wipe is also no longer a deadline for Phase A (§A.0.2), so G4 may fire *before* Phase A without cost |
...
event **three fired**: G5 (deferred by decision), G6 (preempted by a live defect), and **G4** (the wipe,
executed at step 5 of this plan's own order of work). **Phase B is not running.**

> **Count corrected 2026-08-23; it previously read "two fired".** That was accurate when written and stopped
> being so **later in the same plan** — D1 chose the wipe, step 5 executed it, and nothing came back to
> re-read the gate it satisfied. **Only G5 and G6 stop or defer Phase B; G4's effect is the opposite** — it
> *removes* a precondition, leaving Phase B blocked by decision alone. The correction changes no verdict,
> which is exactly why it survived: a stale count with a benign effect has nothing to make it surface.
...
| **D5** | ✅ **DECIDED — the sentinel must exist in RELEASE**, using the project's existing two-half pattern, **verified 2026-08-22**: a release-safe quantity written to a trace in every build, plus a `Conditional("DEBUG")` assertion over the same quantity. Reference: **`AssertBotBallotsVary`** in `NetworkRoot.cs` — grep the symbol — beside the unconditional `spread={2:F1}` emission in `AppendCompanyGovernanceTrace`'s `vote_open` row, immediately after the call. **The pattern is exactly as the developer described it.** *(Cited by symbol, 2026-08-23. This row originally read `:4172`, attribute at `:4279`, emission at `:4173-4178`; all three had drifted +10 to `:4182` / `:4289` / `:4183-4188` within a day of being written. **Standing Convention 15 is exactly this case, and this is the document that generated it** — the line numbers were the only citation in this plan that could rot silently, and they did.)* | A shipped-diagnostics-cost decision | Gate G2, now satisfiable |
