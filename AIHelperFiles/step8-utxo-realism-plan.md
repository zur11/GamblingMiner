...
| 8.2 Satoshi address non-reuse | ◐ **Core done (compiles); spend coupling pending 8.3** | `NodeAgent.ReceiveWallet` (founders satoshi/hal) → coinbase paid to `NextReceiveAddress()`; frontier advances on commit (`MarkReceiveConsumed`), positioned from chain at init (`RescanFounderReceiveWallets`). **Index 0 reserved as base/identity** (genesis, p2p receives); coinbases use index ≥1. `GetNodeSpendableBalance` aggregates owned set + base. ⚠️ **Coupling found:** rotating coinbases off the base starves it, so the **scripted Satoshi spends (E6/E7) break once reached** — the spend path must source from the derived set (8.3). **E7's 82.51 also exceeds a single 50-BTC coinbase**, so under UTXO-lite single-input it must split into **E7a 32.51 + E7b 50.00** (matches the historical E6/E7 amounts). E4 (10) + E6 (32.51) still fit the base-50 and work; only post-E6 spends need 8.3. |
