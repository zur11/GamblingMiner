...
**Status**: ✅ **DONE & in-engine audited** (8.1–8.4 + the full UTXO model of Appendix A + 8.6 docs; 8.5 forensic view deferred). Implements roadmap **Step 8**: *Satoshi-only* fresh derived address per coinbase (address non-reuse) **plus a real multi-input/multi-output UTXO model** (Appendix A, promoted when OQ-8.1's trigger fired), real change outputs to fresh addresses, change rotation for player/casino/Hal/Hearn. Resolved the deferred **E8 (17.49 Hearn change)** from Step 7 and the **§6 address-reuse research**. See ProjectDesignManual Ch. 30.
...
| 8.6 Docs + terminology fix | ✅ **Done (2026-06-29)** | D0 rename applied across the docs (CLAUDE.md balance model; ProjectDesignManual Ch. 30; DESIGN_OVERVIEW, GLOSSARY, PRIVATE_ROADMAP, IMPLEMENTATION_ROADMAP; research §6; btc-wallet-system-plan OQ-2; candidate-block-model-plan). §6 + E8 marked resolved/implemented; roadmap Step 8 checked off; UTXO model recorded (Ch. 30). Carried-forward deferrals (OQ-8.1 multi-input now resolved; OQ-8.2 bots; OQ-8.3 deposit rotation; OQ-8.5 forensic; OQ-8.7 fees) noted everywhere. |
| Post-audit fix — rescan missed change addresses | ✅ **Done (2026-06-29)** | **Cross-session bug** surfaced by a real playtest (a node's change-held funds appeared in-session but vanished after restart; Mike Hearn's 62.51 BTC showed 0; change-address frontier reset → reuse). Root cause: `BuildUsedAddressSet`/`CollectUsedAddressSet` scanned the `tx.Sender`/`tx.Recipient` **shims** (only `Inputs[0]`/`Outputs[0]`), so **change outputs at `Outputs[1]` were invisible** to the chain-rescan → not marked owned. Satoshi was masked (funds on coinbase `Outputs[0]`). **Fix:** iterate the full `Inputs`/`Outputs` lists; same fix in `GetAddressConfirmedTransactions`. No data loss — funds were always on-chain, just unattributed; recovered on next launch. Rule recorded in CLAUDE.md + Ch. 30.1. |
| Wallet transactions + creation dates | ✅ **Done (2026-06-29)** | All non-bot wallets (BTCWallet, CasinoFinances, FoundersWallets) gained a **Transactions panel** (`NetworkRoot.GetNodeTransactionHistory`: per-node sent/received/mined history, dates, counterparty, change netted out, newest-first) with a **"Hide mining rewards" toggle (default on)**, and a **creation date** per address in the address book (`GetNodeAddressBook` now returns first-seen block timestamp). Pattern B scroll; ProjectDesignManual Ch. 29.8 + 30.4. |
...
### A.9 Sequencing & risk — ✅ DONE (history)
- **Risk was high** — it rewrote the validation/signing core that every block and tx flows through. Built on the Step-8 branch with `ChainIsValid` + an in-engine durability/conservation audit as the regression gate (all passed: conservation everywhere, 0 double-spends, 100-input consolidation, full April round-trip + E8).
- **Prereq met**: core Step 8 (8.1–8.4) had shipped, so derived-address wallets + change semantics already existed and the UTXO promotion was mostly bookkeeping — as predicted.
- **Trigger fired exactly as foreseen**: the first time UTXO-lite couldn't fund a send from a single address (a player consolidating many coinbase UTXOs). Promoted then, superseding UTXO-lite.
- **Post-launch fix**: the chain-rescan must scan the full `Inputs`/`Outputs` lists, not the `Sender`/`Recipient` shims (which only see vin/vout 0) — otherwise change addresses vanish from wallets after a restart (see the status-log rows + ProjectDesignManual Ch. 30.1).

---

**Step 8 — only remaining item:** the **Patoshi mining-forensic view (Phase 8.5)** stays **deferred until after Basic Mode v1** (OQ-8.5) — an optional, clearly-labelled cosmetic flavor; the D0 terminology correction already shipped without it. (Other long-term deferrals — bots multi-address OQ-8.2, deposit-address rotation OQ-8.3, network-wide fee activation OQ-8.7 — are independent of Step 8's core and tracked in §6.)

*Created: 2026-06-26; core (8.1–8.4) + full UTXO model (Appendix A) + docs (8.6) DONE & in-engine audited 2026-06-29. Implements roadmap Step 8 (UTXO realism / address non-reuse) on the Step-4 candidate engine + Step-7 founders. Corrected the "Patoshi pattern" misnomer (D0); resolved the §6 address-reuse research + the deferred E8 change output. Appendix A (the full multi-input/multi-output UTXO refactor, OQ-8.1) is now implemented, not just designed. Pairs with `historical-blockchain-events-research.md` and `btc-wallet-system-plan.md`.*
