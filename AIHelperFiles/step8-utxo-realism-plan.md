...
| 8.4 Player wallet UTXO realism | ☐ TODO | **Player keeps a single coinbase address** (spread is Satoshi-only); UTXO realism via **change outputs on send**; BTCWallet aggregated total + address list; send-with-change. |
...
- **OQ-8.3 — player deposit-address rotation.** ✅ **DECIDED — deferred in Basic Mode v1.** Rotating the *incoming* receive address after each external deposit (full HD behavior) is out of scope; the player's UTXO realism is delivered via **change outputs on send** only (coinbase spread is Satoshi-only — Phase 8.4). The hook is noted in Phase 8.4 for a later version.
