...
**Status**: ✅ COMPLETE — v4 (implemented 2026-07-04). All phases SF.0 → SF.5 shipped, user-tested, and documented, plus the added UI phases SF.4B (centralized 3-scope stats + persistent in-DiceGame history), SF.4C (fixed-footer layout) and SF.4D (window maximize + bottom safe-area). Round-1, round-2, and the round-3 model simplification (§0.5) all locked and realized. See the per-phase ✅ markers and the completion note at the end of §4.
...
> **Follow-up fix (2026-07-04) — live stats desync between DiceGame and ScFinances.** After the fact, the `FinancialBettingStats` panel could show a **stale** value in ScFinances vs DiceGame *while the game ran* (self-healed on restart). Root cause: `UserStatsService` throttles `StatsChanged` to 250 ms in high-frequency (autobet) mode and **defers the final batch** until the next bet or a `SetHighFrequencyMode(false)` flush; DiceGame recovers (it toggles that flag and re-`Refresh()`s on entry) but a **passive subscriber like ScFinances** can be left on the last throttled value when betting pauses. Fix: `FinancialBettingStats` now **also refreshes on a `0.75 s` timer in `_Process`** (converge-to-live), so it is correct-by-construction in any host scene regardless of which events it caught — not solely event-driven. Cheap (`Compute` reads `Stats` + two small ledger scans). Data model was never wrong (a fresh boot always recomputes both panels identically — verified against the saved state). Temporary `[STATDIAG]` instrumentation added to confirm, then removed.
...
### Phase SF.5 — Documentation truth pass — ✅ COMPLETED

- [x] **SF.5.1** `CLAUDE.md`: autoloads **12 → 13** + new `PlayerBankAccountService` section; `BankrollProgramService` gains `AutoRechargeEnabled` + session-start-invariant bullets; `CasinoClientLedgerService` updated (Method, `bankroll_withdrawal`, ScFinances, checkpoint coverage); `BlockSessionCheckpointService` now lists **six** restored services; navigation map (ScFinances/ScTransactions + origin-aware back); Canonical Decisions (split kept, new **Private Bank Account** row, game-over → `Bank + Main + Bankroll = 0`); file-tree (`ScFinances/`, `PlayerFinancialStatsCalculator`, DepositPopup removed); Implementation Status (Step 12 entry).
- [x] **SF.5.2** `GLOSSARY.md`: **SC Deposit** rewritten (ScFinances / bank source / manual+auto); new **SC Withdrawal**, **Private Bank Account**, **Casino SC Account**, **Auto-Deposit**, **Auto-Withdraw**, **Net Worth**; **Game over** updated; `Main Balance Manual Recharge` re-tagged `bankroll_withdrawal` (un-planned), `Main Balance Auto-Recharge` cross-referenced to Auto-Withdraw.
- [x] **SF.5.3** `ProjectDesignManual.md`: **new Chapter 32** — three-account topology, the four flows + auto-deposit-as-fallback + auto-withdraw anti-ping-pong, safe-vs-gamblable reserve, NetWorth/OverallPl, ledger taxonomy fix, lifecycle matrix (six-service restore), scenes/navigation/retirements, future `ScBank`.
- [x] **SF.5.4** `DESIGN_OVERVIEW.md` (Economy: player SC finances hub + game-over; §10 P6 note) / `PRIVATE_ROADMAP.md` (P6 mostly-done + auto-withdraw repayment mechanism; P2 auto-recharge toggle shipped): status labels + the P6 casino-repayment note.
- [x] **SF.5.5** *(betting-statistics coverage — SF.4B + the live-sync follow-up)* Documented the `FinancialBettingStats` system in **`ProjectDesignManual.md` §32.9**: the shared 3-scope panel (General / Since last bank deposit / Since last bankroll recharge), the pure `PlayerFinancialStatsCalculator` single-source-of-truth (lifetime from `UserStatsService.Stats`, since-X baselines from client-ledger snapshots — deliberately not the recharge-conflated `ProfitSinceDeposit`), and the **live-sync fix**: the panel refreshes on a `0.75 s` timer (converge-to-live) *in addition to* `StatsChanged`/`LedgerChanged`, because `StatsChanged` is throttled/deferred in high-frequency autobet mode and a passive subscriber (ScFinances) could otherwise show a stale value between the two panels while running (self-healed on restart). The timer is **gated on `SimulationService.IsRunning`** so it only reconciles while an autobet is actively advancing time — when idle, game time doesn't move and immediate events keep the panel current, so the reconcile would be wasted work. Captured the general rule: **live-data panels must not rely solely on a throttled event — add a cheap periodic reconcile, gated on the activity that causes the throttling.** *(Survey note: the other periodic refreshers — 2 s fallback timers in ScFinances/ScTransactions and the DEV casino scenes, plus StatusBar's per-frame refresh — are cheap safety nets / a live HUD and were left as-is.)*

---

## ✅ Step 12 COMPLETE — all phases (SF.0 → SF.5, incl. SF.4B/4C/4D) implemented, tested, and documented.
