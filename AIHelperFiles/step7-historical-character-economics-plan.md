...
| 7.6 Docs | ✅ **Done** | `CLAUDE.md` (FoundersMiningService autoload, founder economics, canonical-decisions rows, implemented list); `ProjectDesignManual.md` **Ch. 28** (founder economics + historical timeline); `GLOSSARY`/`DESIGN_OVERVIEW`/`PRIVATE_ROADMAP`/`PLAYER_GUIDE`; roadmap + founders plan + research doc status flips. |
