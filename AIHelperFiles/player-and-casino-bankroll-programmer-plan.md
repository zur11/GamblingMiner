...
### Phase CG.3 — Bankroll recharge history panel, event timestamps with time, configurable auto-loan amount  ✅ COMPLETE (A/B/C + canonical D) 2026-07-02
...
- [x] **CG.3.D.1** `Scripts/Services/CasinoScBalanceService.cs`: change the two consts — `InitialLoanAmount = 40_000.00000000m`, `DefaultBankroll = 100.00000000m`. Everything else references the constants, so behavior propagates automatically. Update the now-stale inline comments that say "100M" / "1M dose" / "1,000,000" / "100M dose default" / "100M — the auto-loan chunk" (lines ~15, ~227, ~275/276, ~331, ~377/378) to the new figures.
- [x] **CG.3.D.2** `Screens/CasinoGamblingFinances/CasinoGamblingFinances.tscn`: update design-time placeholder texts (cosmetic, overwritten at runtime, but keep them honest): `ManualLoanInput.placeholder_text` (`100000000` → `40000`), `AutoLoanValueLabel` (`100,000,000...` → `40,000...`), `BankrollTargetValueLabel` (`1,000,000...` → `100...`), and the stale `MainBalanceLabel` (`99,000,000...`) / `BankrollLabel` (`1,000,000...`) / `TotalLabel` (`100,000,000...`) — set these last three to `0` / `0` / `0` to match the extra-lazy pre-loan reality anyway.
- [x] **CG.3.D.3** `CLAUDE.md`: update the `CasinoScBalanceService` section (§ around line 196–199: the "100M loan" / dose figures) and **add a Canonical Decisions row**: "Casino defaults — mirror of an average player | auto-loan `40,000 SC`, bankroll dose `100 SC`; first funding reproduces the player's `39,900 / 100` split". Leave the `1,000,000.00000000` number-format example row untouched.
- [x] **CG.3.D.4** `Documentation/ProjectDesignManual.md`: update Ch.31 (§31.1 defaults table, §31.1.1 worked example `Main −1,000,000 → 99,000,000 / Bankroll 999,990`, §31.2 mechanics, and any `100M`/`1M`/`99M` casino mentions) to the new figures; recompute the worked example for the new dose (e.g. Bankroll `0`, player wins `10` → Main `−100 → …`, Bankroll `= 90`).
- [x] **CG.3.D.5** `AIHelperFiles/step11-casino-sc-gambling-finances-plan.md` and this plan file: update casino `99M/1M/100M` mentions to the new figures where they state the canonical defaults (leave historical "why" notes that reference the old values as history, clearly marked).
- [x] **CG.3.D.6** Grep sweep to confirm no stray casino `100,000,000` / `99,000,000` / `1,000,000` default remains (excluding the deliberately-kept number-format example and unrelated non-casino values).
