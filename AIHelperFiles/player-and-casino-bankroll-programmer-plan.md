...
- [ ] **CG.0.3** Call it as the first line of `ApplyBetResult()`:
  ```csharp
...
- [ ] **CG.0.4** Add the pre-genesis reset method, mirroring the player-side services' pattern:
  ```csharp
...
- [ ] **CG.0.5** In `BlockSessionCheckpointService.ResetToPreGenesisDefaults()`, add the casino call alongside the existing player-side resets:
  ```csharp
  GetNodeOrNull<CasinoScBalanceService>("/root/CasinoScBalanceService")
      ?.ResetToPreGenesisDefaults();
  ```

- [ ] **CG.0.6** Extend `BlockSessionCheckpointService.Snapshot` with `CasinoScBankrollTarget` (`decimal`), `CasinoScLoanCount` (`int`), `CasinoScTotalLoaned` (`decimal`), populated in `CaptureCheckpoint()` from `casinoSc.BankrollTarget`/`.LoanCount`/`.TotalLoaned` — closes the post-first-block checkpoint gap described above.

- [ ] **CG.0.7** Extend `CasinoScBalanceService.RestoreCasinoScState(...)` to accept and restore the three new fields (rename/expand its signature as needed), and update `ApplyCheckpointToServices()`'s call site to pass them.
