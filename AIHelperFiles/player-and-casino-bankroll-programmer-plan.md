...
- [ ] **BP.4.1** In `.tscn`, add a new `HBoxContainer` row between `AutoRechargeRow` and `ManualReturnRow`:
  ```
  ...
  ```
- [ ] **BP.4.2** In `.cs`, add private field `private LineEdit _manualRechargeToBankrollInput;`.
- [ ] **BP.4.3** In `_Ready()`:
  ```csharp
