...
| `Scripts/Services/CasinoScBalanceService.cs` | New: `LoanRecord` class, `_loanHistory` list, `CalendarTimeService` reference, `TriggerManualLoan()`, loan logging in `TryAutoRecharge()`, persistence of `LoanHistory` in `Snapshot`. **(Phase CG.0, added 2026-07-01)** New true defaults (`MainBalance=100M` unsplit, `Bankroll=0`, `LoanCount=0`, `TotalLoaned=0`), `EnsureInitialCasinoFundingIfNeeded()`, `ResetToPreGenesisDefaults()` — see `Documentation/ProjectDesignManual.md` §31.1 |
...
### Phase CG.0 — CasinoScBalanceService: pre-genesis / first-bet lifecycle parity with the player
